...
__STATIC_INLINE const char *devIdToStr(uint16_t dev_id) {
	...
	case 0x449:
		...
		break;
	case 0x450:
		str = "STM32H743/753 and STM32H750";
		break;
	default:
		str = "UNKNOWN";
	}
...
__STATIC_INLINE void logDeviceConf(void) {
	...
	printf(" Device       : DevID:0x%08x (%s) RevID:0x%08x\r\n",
			(int) HAL_GetDEVID(), devIdToStr(HAL_GetDEVID()),
			(int) HAL_GetREVID());
...
#if !defined(STM32F3)
	printf(
			" FLASH conf.  : ACR=0x%08x - Prefetch=%s $I/$D=(%s,%s) latency=%d\r\n",
			(int) acr,
			bitToStr((acr & FLASH_ACR_PRFTEN_Msk) >> FLASH_ACR_PRFTEN_Pos),
...
static int ioGetUint8(uint8_t *buff, int count, uint32_t timeout) {
	...
	status = HAL_UART_Receive(&UartHandle, (uint8_t *) buff, count, timeout);
...
__STATIC_INLINE void aiPrintLayoutBuffer(const char *msg,
	...
	printf("%s HWC layout:%d,%d,%ld (s:%ld f:%s)\r\n", msg, buffer->height,
			buffer->width, buffer->channels, aiBufferSize(buffer),
...
static int aiTestPerformance(int idx) {
	...
	for (iter = 0; iter < niter; iter++) {
		...
		batch = ai_mnetwork_run(net_ctx[idx].handle, &ai_input[0],
				&ai_output[0]);
		ai_float* output_ = (ai_float *) (ai_output[0].data);
	...
	if (__get_MSP() != cstack) {
		printf(
				"E: !current stack address is not coherent 0x%08lx instead 0x%08lx\r\n",
				__get_MSP(), cstack);
	}
#endif
	...
	printf(
			"Results for \"%s\", %d inferences @%ldMHz/%ldMHz (complexity: %lu MACC)\r\n",
			net_ctx[idx].report.model_name, iter,
			HAL_RCC_GetSysClockFreq() / 1000000,
			HAL_RCC_GetHCLKFreq() / 1000000, net_ctx[idx].report.n_macc);
...
#if _APP_HEAP_MONITOR_ == 1
	printf(
			" used heap    : %ld:%ld %ld:%ld (req:allocated,req:released) cfg=%ld\r\n",
			ia_malloc.alloc_req, ia_malloc.alloc, ia_malloc.free_req,
			ia_malloc.free, (ia_malloc.cfg & (3 << 1)) >> 1);
#else
