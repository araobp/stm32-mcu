...
Refer to "aiSystemPerformance.c" that is generated by selecting "System Performance" on CubeMX. Maybe it is a good idea to use the code as a basis for developing an application.
...
"aiSystemPerformance.c" just uses the following input data (float) for performance testing:
```
     /* Fill input vector */
...
Note: I did the same scaling (-1.0 to 1.0) on dataset for training CNN on Keras.
